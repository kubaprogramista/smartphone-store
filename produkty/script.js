...
function dark_mode(){
    if(toggle.checked){
        ...
        document.body.style.backgroundSize = "145%";
...
const renderProducts = (items) => {
    ...
    for(let i = 0; i< items.length; i++){
        ...
        newProduct.className = `productBox ${items[i].category} ${items[i].sale ? 'onSale' : ''}`;
        ...
        <button class="addToCartBtn ${items[i].id}"><img src="../png/add-to-cart.png" alt="add to cart"></button>
    ...
    addToCartBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            newCartProduct.innerHTML = `<img src="${products[productID].image}" alt="product image">
            <section class="cart-product-info">
                <p class="cart-product-name">${products[productID].name}</p>
                <p class="cart-product-price">${(products[productID].sale ? (products[productID].price - products[productID].saleAmount).toFixed(2) : products[productID].price.toFixed(2))} zł</p>
            </section>`;
            itemPrice = (products[productID].sale ? (products[productID].price - products[productID].saleAmount).toFixed(2) : products[productID].price.toFixed(2));
...
cartClearBtn.addEventListener('click', () => {
    ...
    cartSection.innerHTML = "<div class='cart-if-empty'>Twój koszyk jest pusty...</div>";
    totalPrice=0;
    countItems=0;
